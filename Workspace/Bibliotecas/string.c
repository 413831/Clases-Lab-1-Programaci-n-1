...
int string_getStringLetras(char input[],int size,char mensaje[],char msjError[])
{
    ...
    if(string_getString(mensaje,size,buffer) == 0 && validacion_Letras(buffer))
    {
 ...
 */
int string_getStringNumeros(char input[],int size,char mensaje[],char msjError[],int minimo,int maximo,int reintentos)
{
    char buffer[size];
    int retorno = 1;
       ...
       msjError != NULL && reintentos >= 0)
    {
        do
        {
            reintentos--;
            printf("%s",mensaje);
            if(string_getString(mensaje,size,buffer) == 0 &&
                validacion_Int(buffer,size,minimo,maximo))
            {
                strncpy(input,buffer,size);//Se copia string cargado a variable local
                retorno = 0;
