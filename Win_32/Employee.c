...
int employee_disable(void* pArrayListEmployee)
{
    Employee* this = NULL;
    int retorno = -1;
    char bufferId[1000];
   ...
   if(!employee_input("ID",bufferId,1000,isValidId))
    {
        ...
        if(this != NULL)
        {
            ...
            {
                employee_setIdEmpty(this);//Seteo ID en -1
                retorno = 0;
            }
        }
        else
...
Employee* employee_newConParametros(char* id,char* nombre,char* horasTrabajadas,char* sueldo)
{
    ...
    else if(!strcmp("-1",id) && !employee_setId(this,id))
    {
        return this;
    }
    else
...
int employee_setIdEmpty(Employee* this)
{
    int retorno=-1;
    if(this!=NULL)
    {
...
int employee_setId(Employee* this,char* id)////////VALIDAR ID INICIAL CONTRA ARCHIVO
{
    ...
    if(this!=NULL && auxId == 0)//Asigno primer ID
    {
        proximoId++;
        this->id=proximoId;
        retorno=0;
    }
    else if(auxId>proximoId)//Asigno ID correlativamente
    {
        proximoId=auxId;
        this->id=proximoId;
        retorno=0;
    }
    else if(auxId==-1)//Asigno ID inhabilitado
    {
        this->id=-1;
    }
    return retorno;
...
int employee_listDisabled(Employee* this)
{
    int retorno = -1;
    int auxId;
    char auxNombre[128];
